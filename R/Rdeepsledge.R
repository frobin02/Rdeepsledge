...
PICTOBIIGLE <- function(mydir) {
  if (!is.null(input)) {
    setwd(input)
    pic_files <- list.files(pattern = ".png")
    l <- as.data.frame(t(pic_files))
    write.table(l, "list.file.grid.txt", sep = ',', col.names = FALSE, row.names = FALSE, quote = FALSE)
    message("File list successfully saved to list.file.grid.txt")
  } else {
...
TODARCKPIC <- function(mydir,sensibility,plot){
datablack<-list(NULL)
setwd(mydir)
